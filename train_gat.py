...
def train_gat():
    ...
    model = GAT(num_users, num_items).to(device)
    ...
    torch.save(user_emb.detach().cpu(), "data/trained_user_emb_gat.pt")
    torch.save(item_emb.detach().cpu(), "data/trained_item_emb_gat.pt")
    joblib.dump(user_encoder, "data/user_encoder.pkl")
    joblib.dump(song_encoder, "data/song_encoder.pkl")
    print(" Trained GAT embeddings and encoders saved!")
